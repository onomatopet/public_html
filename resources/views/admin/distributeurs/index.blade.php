    ...
    <div class="px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-lg shadow-sm px-6 py-4 mb-6">
            <nav class="flex items-center text-sm">
                ...
                <span class="text-gray-700 font-medium">Distributeurs</span>
            </nav>
        </div>
        ...
        <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Distributeurs</h1>
                <p class="mt-2 text-gray-600">Gérez les distributeurs de votre réseau MLM</p>
            </div>
            <div class="mt-4 sm:mt-0">
                <a href="{{ route('admin.distributeurs.create') }}"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <div class="bg-white rounded-lg shadow-sm p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-gray-600">Total distributeurs</p>
                        <p class="text-2xl font-semibold text-gray-900">{{ $distributeurs->total() }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-semibold text-gray-900">{{ number_format($avgGrade ?? 1, 1) }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-2xl font-semibold text-gray-900">{{ $newThisMonth ?? 0 }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
            <form method="GET" action="{{ route('admin.distributeurs.index') }}" class="space-y-4">
        ...
        <div class="bg-white rounded-lg shadow-sm overflow-hidden">
            @if($distributeurs->count() > 0)
                            ...
                            @foreach($distributeurs as $distributeur)
                                <tr class="hover:bg-gray-50 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="text-sm text-gray-500">{{ $distributeur->adress_distributeur ?? '-' }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <div class="flex items-center justify-center">
                                            @for($i = 1; $i <= $distributeur->etoiles_id; $i++)
                                                <svg class="w-4 h-4 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
                                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                                </svg>
                                            @endfor
                                        </div>
                                        <span class="text-xs text-gray-500">Niveau {{ $distributeur->etoiles_id }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if($distributeur->parent)
                                            <div class="text-sm text-gray-900">{{ $distributeur->parent->full_name }}</div>
                                            <div class="text-sm text-gray-500">#{{ $distributeur->parent->distributeur_id }}</div>
                                        @else
                                            <span class="text-sm text-gray-500 italic">Aucun parrain</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-2">
                                            ...
                                            <form action="{{ route('admin.distributeurs.destroy', $distributeur) }}"
                                                  method="POST"
                                                  class="inline"
                                                  onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce distributeur ?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                ...
                <div class="bg-white px-4 py-3 border-t border-gray-200 sm:px-6">
                    {{ $distributeurs->withQueryString()->links() }}
                </div>
            @else
                {{-- Message vide --}}
                <div class="text-center py-12">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-gray-900">Aucun distributeur</h3>
                    <p class="mt-1 text-sm text-gray-500">
                        @if(request()->anyFilled(['search', 'grade', 'status']))
                            Aucun distributeur ne correspond à vos critères de recherche.
                        @else
...
<script>
    ...
    document.querySelectorAll('select[name="grade"], select[name="status"]').forEach(select => {
